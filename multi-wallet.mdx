...
To simplify management of these scenarios and the resulting edge cases, we've released a widget that does it for you. You can see it in action on our [demo site](https://demo.dynamic.xyz).

### Setup

#### multiWallet (optional)

The `multiWallet` prop is a boolean, that by default is set to `false`. If you want to enable it, set the prop to `true`.
