...
1. Go to the your environment's dashboard [Developer Tab](https://app.dynamic.xyz/dashboard/developer)
2. In the "API Token" section, click on "Create Token"
3. Provide a name for the token. The best practice here would be to name the token after the system you intend to use this token with. A few examples: "Mycompany Admin", "Ben's personal token", or "background-job service".
...
Use the token generated through dashboard to programmatically access Dynamic's API by adding it to the `Authentication` header of your HTTP request.
